...
#
# Модель для создания запроса о подтверждении оплаты системы WebPay. Перед созданием обязательно создайте клиента.
#
# Запрос к тестовой среде отсылается на адрес https://sandbox.webpay.by, а к реальной среде - на https://billing.webpay.by
# (это учитывается автоматически, в зависимости от значения debug_mode клиента).
#
# Пример:
#
