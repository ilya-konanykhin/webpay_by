...
# указанному в поле wsb_notify_url (WebpayBy::Request#notify_url). В этом запросе содержится информация по платежу.
#
# Полученную информацию Интернет-ресурс должен проверить в соответствии с требованиями выполнения запроса
# и ответить кодом: "HTTP/1.0 200 OK".
#
# Эта модель служит для работы с полученным от сервера WebPay запросом. Перед созданием объекта обязательно создайте
# клиента.
#
# Поля, содержащиеся в запроса сервера:
#   batch_timestamp - время совершения транзакции
#   currency_id     - валюта транзакции
#   amount          - сумма транзакции
#   payment_method  - метод совершения транзакции
#   order_id        - номер запроса в системе WebPay
#   site_order_id   - номер заказа (не запроса!), присвоенный магазином (WebpayBy::Request#order_id)
